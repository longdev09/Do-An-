...
import style from "./PreviewCourseAdmin.module.scss";
import { Button } from "antd";
import routes from "../../config/routeConfig";
const cx = classNames.bind(style);

export default function PreviewCourseAdmin() {
  const ulr = useParams();
  ...
  let nav = useNavigate();
  const handleDuyetKhoaHoc = async () => {
    await updateTrangThaiKhoaHoc(
      ulr.makh,
      "Đang bán",
      "Khóa học đã được duyệt",
      "Khóa học đã từ chối duyệt"
    );
    nav(routes.admin_Courses);
  };
          ...
          <div className={cx("container")}>
            <div className={cx("heading")}>
              ...
              <div className={cx("action")}>
                <Button
                  onClick={handleDuyetKhoaHoc}
                  style={{ marginRight: "10px" }}
                  type="primary"
                >
                  Duyệt
                </Button>
                <Button style={{ marginRight: "30px" }}>Từ Chối</Button>
              </div>
            </div>
